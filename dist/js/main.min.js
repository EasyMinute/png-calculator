...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      var formLoading = form.querySelector('#png-calculator__loading');
      ...
      // Append files
      var fileInput = document.querySelector('#calc_print_files');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var productFinalPriceInput = document.getElementById('product_final_price');
  var productSumInput = document.getElementById('product_sum');
  var productPriceKoefs = pngCalculatorData.productPriceKoefs;
  var productDiscountKoefs = pngCalculatorData.productDiscountKoefs;
  ...
  function calculateFinalPriceAndSum() {
    var productQuantity = parseInt(productQuantityInput.value) || 0;
    ...
    var koef = null;
    productPriceKoefs.forEach(function (priceRange) {
      if (productPrice >= priceRange.price_min && productPrice <= priceRange.price_max) {
        priceRange.quantity_rows.forEach(function (quantityRange) {
    ...
    var productFinalPrice = productPrice * koef;

    // ✅ Apply discount if a discount is selected
    var discountSelect = document.querySelector('.prodDiscounts');
    if (discountSelect) {
      var selectedDiscountId = discountSelect.value; // Get selected discount ID

      // Find the selected discount object in productDiscountKoefs
      var selectedDiscount = productDiscountKoefs.find(function (discount) {
        return discount.id === selectedDiscountId;
      });
      if (selectedDiscount) {
        // Find the discount value based on price bounds
        var discountValue = selectedDiscount.values.find(function (valueRange) {
          return productPrice >= parseFloat(valueRange.price_min) && productPrice <= parseFloat(valueRange.price_max);
        });
        if (discountValue) {
          var discountMultiplier = parseFloat(discountValue.value);
    ...
    var productSum = productFinalPrice * productQuantity;
  ...
  var maxPrints = 5;
  var currentPrints = 0;
  var printGroupsContainer = document.getElementById('printGroupsContainer');
  var addPrintButton = document.getElementById('addPrint');
  var removePrintButton = document.getElementById('removePrint');
  var printFinalPriceInput = document.getElementById('print_final_price');
  var printSumInput = document.getElementById('print_sum');
  var koefsMap = {
  ...
  var urgencyKoefs = pngCalculatorData.additionalUrgency;

  // Initialize Select2 on all printType and printFormat dropdowns
  function initializeSelect2(group) {
    if (group) {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.printType')).select2({
        placeholder: 'Оберіть тип друку',
        allowClear: true,
        width: '100%',
        minimumResultsForSearch: Infinity
      });
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.printFormat')).select2({
      ...
      var $discounts = jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.printDiscounts')).select2({
      ...
      // Initialize Select2 for the printDiscounts dropdown
      var $discountsprod = jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.prodDiscounts')).select2({
        placeholder: 'Оберіть знижку',
        allowClear: true,
        width: '100%',
        minimumResultsForSearch: Infinity // Disable search
  ...
  function tapePrintCalculate(printWidth, printLength, printType) {
    ...
    var printQuantity = parseInt(document.getElementById('product_quantity').value) || 0;

    // Exit early if printType not found
    if (!printTypeOptions[printType]) return 0;
    var typeOptions = printTypeOptions[printType];

    // Convert all numeric inputs from strings to floats
    var distance = parseFloat(typeOptions.distance) || 0;
    var tapeWidth = parseFloat(typeOptions.tape_width) || 0;
    var tapePrice = parseFloat(typeOptions.tape_price) || 0;
    printWidth = parseFloat(printWidth) || 0;
    printLength = parseFloat(printLength) || 0;

    // Predefine Variables for calculations
    var printsQuantOrigin = 0;
    var printsQuantReverse = 0;
    var columsQuantOrigin = 0;
    var columsQuantReverse = 0;
    var linearLengthOrigin = 0;
    var linearLengthReverse = 0;
    var optimalLength = 0;
    var tapeLength = 0;
    var printPrice = 0;

    // Calculations
    var effectiveWidth = printWidth + distance;
    var effectiveLength = printLength + distance;
    ...
    var totalTapePrice = tapeLength * tapePrice;
    printPrice = totalTapePrice;
    var applyPrice = 0;
    // Add applying_price (per-item addition)
    if (Array.isArray(typeOptions.applying_price)) {
      var _iterator = _createForOfIteratorHelper(typeOptions.applying_price),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var range = _step.value;
          var min = parseInt(range.quantity_min);
          var max = parseInt(range.quantity_max);
          var price = parseFloat(range.price);
    ...
    var pricePerPrint = printPrice / printQuantity + applyPrice;
    return pricePerPrint;
  }
  function changeSizes() {
    var selects = document.querySelectorAll('.printFormat');
    selects.forEach(function (select) {
      var selectedOption = select.options[select.selectedIndex];
      var value = selectedOption.value;
      var group = select.closest('.pngcalc__block__group');
      var widthInput = group.querySelector('input.weight');
      var lengthInput = group.querySelector('input.length');
      if (value === 'custom') {
        widthInput.value = 0;
        lengthInput.value = 0;
        ...
        var width = selectedOption.getAttribute('data-width');
        var length = selectedOption.getAttribute('data-leght'); // Note: 'data-leght' as in your HTML
  ...
  function calculatePrintCosts() {
    ...
    var printGroups = Array.from(printGroupsContainer.querySelectorAll('.pngcalc__block__group'));
    var totalPrice = 0;

    // Base calculation for print groups
    printGroups.forEach(function (group) {
      var printType = jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.printType')).val();
      var printFormat = jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.printFormat')).val();
      var printWidth = jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.weight')).val();
      var printLength = jquery__WEBPACK_IMPORTED_MODULE_0___default()(group.querySelector('.length')).val();
    ...
    var baseFinalPrice = totalPrice.toFixed(2); // Base price per item
    var baseSum = parseFloat((baseFinalPrice * productQuantity).toFixed(2)); // Base sum before discounts or surcharges

    var finalSum = baseSum;
    ...
    if (document.getElementById('printUrgency')) {
      ...
      if (document.getElementById('printUrgency').checked) {
        var _urgencyKoefs = pngCalculatorData.additionalUrgency;
        var matchedUrgencyKoef = _urgencyKoefs.find(function (range) {
          return productQuantity >= parseInt(range.quantity_min) && productQuantity <= parseInt(range.quantity_max);
        });
    ...
    var printClientProduct = document.getElementById('printClientProduct');
    if (document.getElementById('printClientProduct')) {
      var _printClientProduct = document.getElementById('printClientProduct').checked ? parseFloat(document.getElementById('printClientProduct').value) || 1 : 1;
      finalSum += baseSum * (_printClientProduct - 1); // Adjust for client product multiplier
    }

    if (document.getElementById('printDifficulty')) {
      var printDifficulty = document.getElementById('printDifficulty').checked ? parseFloat(document.getElementById('printDifficulty').value) || 1 : 1;
    ...
    if (document.getElementById('printPackaging')) {
      var printPackaging = document.getElementById('printPackaging').checked ? parseFloat(document.getElementById('printPackaging').value) || 0 : 0;
      finalSum += printPackaging * productQuantity; // Add packaging costs based on quantity
    }

    // Apply discount
    if (document.querySelector('.printDiscounts')) {
      var printDiscount = parseFloat(document.querySelector('.printDiscounts').value) || 1; // Default to no discount
  ...
  function createPrintGroup() {
    var template = document.querySelector('.print-group-template');
    ...
    var printTypeSelect = newGroup.querySelector('.printType');
    var printFormatSelect = newGroup.querySelector('.printFormat');
    var printSizesInputWeight = newGroup.querySelector('.sizes_inputs_input.weight');
    var printSizesInputLength = newGroup.querySelector('.sizes_inputs_input.length');
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(printTypeSelect).on('change', function () {
      // populatePrintFormats(printTypeSelect.value, printFormatSelect);
      calculatePrintCosts();
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(printSizesInputWeight).on('change', function () {
      calculatePrintCosts();
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(printSizesInputLength).on('change', function () {
      calculatePrintCosts();
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(printSizesInputWeight).on('input', function () {
      calculatePrintCosts();
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(printSizesInputLength).on('input', function () {
      calculatePrintCosts();
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(printFormatSelect).on('change', function () {
      changeSizes();
      calculatePrintCosts();
    });
  ...
  var previousProductPrice = productPriceInput.value; // Store the original price
  var printClientProductCheckbox = document.getElementById('printClientProduct');
  ...
  function updateTotalSpans() {
    var productFinalPrice = parseFloat(productFinalPriceInput.value) || 0;
    var productSum = parseFloat(productSumInput.value) || 0;
    var printFinalPrice = parseFloat(printFinalPriceInput.value) || 0;
    var printSum = parseFloat(printSumInput.value) || 0;

    // Calculate totalPrice and totalSum
    var totalPrice = productFinalPrice + printFinalPrice;
    var totalSum = productSum + printSum;
  ...
  removePrintButton.addEventListener('click', function (e) {
    e.preventDefault();
    if (currentPrints > 1) {
      var lastGroup = printGroupsContainer.lastElementChild;
  ...
  if (document.querySelectorAll('.pngcalc_label.checkbox input[type="checkbox"]')) {
    document.querySelectorAll('.pngcalc_label.checkbox input[type="checkbox"]').forEach(function (checkbox) {
  ...
  function initializePrintGroups() {
    var existingGroups = Array.from(document.querySelectorAll('.pngcalc_label.select'));
...
document.addEventListener('DOMContentLoaded', function () {
  var form = document.getElementById('tape-calculator');
  if (form) {
    var handleInputChange = function handleInputChange() {
      // Get body input values
      var printWidth = parseFloat(document.getElementById('print_width').value) || 0;
      var printLength = parseFloat(document.getElementById('print_length').value) || 0;
      var printQuantity = parseInt(document.getElementById('print_quantity').value) || 0;
      var cuttingEnabled = cuttingCheckbox.checked;
      ...
      var optimalLength = 0;
      //Final values
      var optimalOrientation = 'По ширині';
      var tapeLength = 0;
      var printPrice = 0;
      var totalTapePrice = 0;
    ...
    var bodyInputs = form.querySelectorAll('.tape-calculator__body input');
    var cuttingCheckbox = document.getElementById('tape-cutting');
    var hiddenInputs = {
...
(jquery__WEBPACK_IMPORTED_MODULE_0___default().fn).inputmask = function (options) {
  return this.each(function () {
    inputmask__WEBPACK_IMPORTED_MODULE_1___default()(options).mask(this);
  });
};
jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).ready(function () {
  // Apply phone mask
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(".required-phone").inputmask("+38 (099) 999 99 99");
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()("#submitCalc, .pngcalc_button.pngcalc_stepper").on("mouseup", function (e) {
    e.preventDefault();
    e.stopPropagation();
    var isValid = true;

    // Find the current step
    var $currentStep = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).closest(".pngcalc__step"); // Adjust class if needed

    // Validate only fields in the current step
    $currentStep.find(".required").each(function () {
      validateField(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this), jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).val().trim() !== "");
      if (jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).hasClass("invalid")) isValid = false;
    });

    // Validate email
    $currentStep.find(".required-email").each(function () {
      var email = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).val().trim();
      var emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      validateField(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this), emailPattern.test(email));
      if (jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).hasClass("invalid")) isValid = false;
    });

    // Validate phone
    $currentStep.find(".required-phone").each(function () {
      validateField(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this), inputmask__WEBPACK_IMPORTED_MODULE_1___default().isValid(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).val(), {
    ...
    $currentStep.find("#calc_print_files").each(function () {
      ...
      if (files.length > maxFiles) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).addClass("invalid");
        this.value = "";
        return;
      }
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()("input").on("input", function () {
...
/*!********************!*\
  ...
  \********************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _blocks_calc_general__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./blocks/calc_general */ "./js/blocks/calc_general.js");
/* harmony import */ var _blocks_calc_general__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_blocks_calc_general__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _blocks_calculator__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./blocks/calculator */ "./js/blocks/calculator.js");
/* harmony import */ var _blocks_tape_calculator__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./blocks/tape-calculator */ "./js/blocks/tape-calculator.js");
/* harmony import */ var _blocks_tape_calculator__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_blocks_tape_calculator__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _blocks_validation__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./blocks/validation */ "./js/blocks/validation.js");
/* harmony import */ var _blocks_ajax_sending__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./blocks/ajax-sending */ "./js/blocks/ajax-sending.js");
/* harmony import */ var _blocks_ajax_sending__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_blocks_ajax_sending__WEBPACK_IMPORTED_MODULE_4__);
